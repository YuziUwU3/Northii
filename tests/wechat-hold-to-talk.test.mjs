...
assert.match(app,/onclick="_voiceMode=!_voiceMode;render\(\)"/);
assert.match(app,/svgIc\('mic',19,_voiceMode\?'#f3f3f4':'#9b9ca0'\)/);
assert.match(app,/_voiceMode\?'输入文字，发送为语音条…':'发消息…'/);
assert.match(app,/>\$\{_voiceMode\?'发语音':'发送'\}<\/button>/);
assert.match(app,/let _voiceMode=false;let _panelPage='fn';/);
