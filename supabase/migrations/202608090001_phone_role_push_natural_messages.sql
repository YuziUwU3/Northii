alter table public.phone_role_push_profiles
  add column if not exists message_min smallint not null default 1 check (message_min between 1 and 10),
  add column if not exists message_max smallint not null default 4 check (message_max between 1 and 10);
...
begin
  ...
  insert into public.phone_role_push_profiles (
    ...
    message_min, message_max, next_due_at, updated_at
  ) values (
    v_target,
    v_role_id,
