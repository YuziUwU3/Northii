-- v892: 0 means daily-count-driven random contact; every new conversation
-- activity still imposes an independent random 30-to-60-minute quiet period.
alter table public.phone_role_push_profiles
  drop constraint if exists phone_role_push_profiles_idle_minutes_check;

alter table public.phone_role_push_profiles
  add constraint phone_role_push_profiles_idle_minutes_check
  check (idle_minutes between 0 and 1440);

alter table public.phone_role_push_profiles
  add column if not exists quiet_until_at timestamptz;
...
drop function if exists public.phone_role_push_touch_activity(text, text, text, text, text, bigint);
...
declare
  ...
  v_quiet_until timestamptz := case when coalesce(p_quiet_until_ms, 0) > 0
    then to_timestamp(p_quiet_until_ms::numeric / 1000) else null end;
begin
  if not public.phone_companion_owner_ok(v_target, p_owner_secret) then return false; end if;
  if v_role_id = '' then raise exception 'invalid-role-id'; end if;
  ...
  set recent_context = left(trim(coalesce(p_recent_context, '')), 8000),
      ...
      quiet_until_at = v_quiet_until,
      next_due_at = case when enabled then v_activity + make_interval(mins => 30 + floor(random() * 31)::integer) else null end,
      claimed_until = null,
      updated_at = now()
...
revoke all on function public.phone_role_push_touch_activity(text, text, text, text, text, bigint, bigint) from public;
grant execute on function public.phone_role_push_touch_activity(text, text, text, text, text, bigint, bigint) to anon, authenticated;
