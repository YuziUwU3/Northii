...
OUTPUT = Path(sys.argv[1]) if len(sys.argv) > 1 else ROOT / "v868-maintenance-sections.pdf"
MARKERS = {
    "AI开发项目_项目说明文档.docx": "v868｜共同生活位置与一起外出同步（2026-08-10）",
    "AI开发项目_Bug记录模板.docx": "v868 Bug 记录｜地点不跟随与一起外出被误判为单独外出（2026-08-10）",
    "AI开发项目_Bug修改规范.docx": "新增强制规范｜共同生活阶段、同场关系与位置必须分离（v868 起）",
    "AI开发项目_新聊天启动说明.docx": "新聊天接手状态｜v868 共同生活位置与一起外出（2026-08-10）",
}
...
SimpleDocTemplate(str(OUTPUT), pagesize=A4, leftMargin=18 * mm, rightMargin=18 * mm, topMargin=16 * mm, bottomMargin=16 * mm, title="v868 maintenance sections").build(story)
print(OUTPUT)
