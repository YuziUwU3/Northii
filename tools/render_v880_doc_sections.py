...
OUTPUT = Path(sys.argv[1]) if len(sys.argv) > 1 else ROOT / ".qa" / "v880-maintenance-sections.pdf"
MARKERS = {
    "AI开发项目_项目说明文档.docx": "v880｜持久锁定与当日同步（2026-08-11）",
    "AI开发项目_Bug记录模板.docx": "v880 Bug 记录｜刷新误解锁、假手动解锁与跨日旧使用量（2026-08-11）",
    "AI开发项目_Bug修改规范.docx": "新增强制规范｜锁定意图与使用量快照必须分层（v880 起）",
    "AI开发项目_新聊天启动说明.docx": "新聊天接手状态｜v880 持久锁定与当日同步（2026-08-11）",
}
...
SimpleDocTemplate(str(OUTPUT), pagesize=A4, leftMargin=18 * mm, rightMargin=18 * mm, topMargin=16 * mm, bottomMargin=16 * mm, title="v880 maintenance sections").build(story)
print(OUTPUT)
