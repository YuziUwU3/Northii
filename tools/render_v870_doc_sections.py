...
OUTPUT = Path(sys.argv[1]) if len(sys.argv) > 1 else ROOT / ".qa" / "v870-maintenance-sections.pdf"
MARKERS = {
    "AI开发项目_项目说明文档.docx": "v870｜主辅模型统一 API 路线与就近保存（2026-08-10）",
    "AI开发项目_Bug记录模板.docx": "v870 Bug 记录｜API 路线只保存主模型且保存入口过远（2026-08-10）",
    "AI开发项目_Bug修改规范.docx": "新增强制规范｜一条 API 路线必须原子保存主辅模型（v870 起）",
    "AI开发项目_新聊天启动说明.docx": "新聊天接手状态｜v870 主辅模型统一 API 路线（2026-08-10）",
}
...
SimpleDocTemplate(str(OUTPUT), pagesize=A4, leftMargin=18 * mm, rightMargin=18 * mm, topMargin=16 * mm, bottomMargin=16 * mm, title="v870 maintenance sections").build(story)
print(OUTPUT)
