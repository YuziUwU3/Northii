...
OUTPUT = Path(sys.argv[1]) if len(sys.argv) > 1 else ROOT / ".qa" / "v869-maintenance-sections.pdf"
MARKERS = {
    "AI开发项目_项目说明文档.docx": "v869｜共同生活限额与每日必查（2026-08-10）",
    "AI开发项目_Bug记录模板.docx": "v869 Bug 记录｜共同生活缺少限额与每日必查交接（2026-08-10）",
    "AI开发项目_Bug修改规范.docx": "新增强制规范｜跨通道每日必查必须显式交接（v869 起）",
    "AI开发项目_新聊天启动说明.docx": "新聊天接手状态｜v869 共同生活限额与每日必查（2026-08-10）",
}
...
SimpleDocTemplate(str(OUTPUT), pagesize=A4, leftMargin=18 * mm, rightMargin=18 * mm, topMargin=16 * mm, bottomMargin=16 * mm, title="v869 maintenance sections").build(story)
print(OUTPUT)
